...
def get_heightmap(color_img, depth_img,
                  ...
                  workspace_limits, heightmap_resolution=0.005):
...
def euler2rotm(theta):
    ...
    R_y = np.asarray([[np.cos(theta[1]),    0,      np.sin(theta[1])],
                      [0,                     1,      0              ],
...
def cal_relative_theta(prev_ori_mat, cur_ori_mat):
    """
    :param prev_ori_mat: numpy_array
    :param cur_ori_mat: numpy_array
    :return:
    """
    r_mat_1 = euler2rotm(prev_ori_mat)
    r_mat_2 = euler2rotm(cur_ori_mat)
    rot_mat = np.dot(r_mat_1.T, r_mat_2)
